...
private:
    ...
    void build(int u,int L,int R,vector<type>&a){
        ...
        if(L==R){
            w[u].sum=a[L-1],w[u].p_sum=Pow(a[L-1]);
            return;
        }
        int M=(L+R)/2;
    ...
    void make_tag(int u,type k){
        ...
        w[u].p_sum=w[u].p_sum+2*k*w[u].sum+len*k*k;
        w[u].sum=k*len+w[u].sum;
        return;
    }
    //tag下放
    ...
    void pushup(int u){
        ...
        w[u].p_sum=w[2*u].p_sum+w[2*u+1].p_sum;
        return;
    }
public:
    //构造函数
    ...
    //区间查询
    type query_sum(int u,int L,int R){//[L,R]待查询的区间，w[u]被查询的节点
    ...
    type query_p_sum(int u,int L,int R){//[L,R]待查询的区间，w[u]被查询的节点
        if(in_range(w[u].l,w[u].r,L,R)){
            return w[u].p_sum;
        }
        else if(!out_range(w[u].l,w[u].r,L,R)){
            push_down(u);
            return query_p_sum(2*u,L,R)+query_p_sum(2*u+1,L,R);
        }
        else{
            return 0;
        }
...
int main()
{
    ...
    while(m--){
        ...
        if(op==1){
            ...
            int x,y;cin>>x>>y;
            double sum=st.query_sum(1,x,y);
            int len=y-x+1;
            cout<<sum/len<<endl;
        }
        else if(op==3){
            int x,y;cin>>x>>y;
            double p_sum=st.query_p_sum(1,x,y);
            double sum=st.query_sum(1,x,y);
            int len=y-x+1;
            double ans=p_sum/len-Pow(sum/len);
            cout<<ans<<endl;
        }
    }
    return 0;
